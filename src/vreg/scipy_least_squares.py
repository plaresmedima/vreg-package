...
def least_squares(
        ...
        f_scale=1.0, diff_step=None, abs_step=None, tr_solver=None, tr_options={},
        jac_sparsity=None, max_nfev=None, verbose=0, args=(), kwargs={}):
    """Solve a nonlinear least-squares problem with bounds on the variables.

    **NOTE**: This is scipy's least_squares function with one keyword argument
    ``abs_step`` added to expose the option of providing an absolute step size for 
    ...
    if callable(jac):
        ...
        if method == 'lm':
            ...
            def jac_wrapped(x, f):
                J = approx_derivative(fun, x, rel_step=diff_step, 
                                      abs_step=abs_step, method=jac,
